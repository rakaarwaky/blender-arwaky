#!/usr/bin/env python3
"""Build the Blender Arwaky addon ZIP package.

Usage:
    uv run python scripts/build/build_addon_package.py

Output: ``dist/blender_mcp_addon.zip`` (consumed by the release workflow).
"""
import os
import zipfile
import sys
...
def build_addon():
    # Setup paths relative to this script (lives in scripts/build/)
    ...
    # Build the ZIP in dist/ (standard Python package convention)
    ...
    ignore_patterns = {
        ...
        ".git",
        ".DS_Store",
        "blender_mcp_addon.zip",  # exclude previous build artifact
    }
