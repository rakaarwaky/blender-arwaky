...
- material_mode (Literal['STROKE', 'FILL', 'BOTH']) – Material Mode, (optional)   Returns: 
...
set[Literal[[Operator Return Items](bpy_types_enum_items/operator_return_items.html#rna-enum-operator-return-items)]]
      ...
      bpy.ops.ui.view_item_navigate(*, direction='UP') 
...
Navigation Direction, Direction in which to navigate and select next element. (optional)

  
- `UP` Up – Select item above the active. 
- `DOWN` Down – Select item below the active. 
- `LEFT` Left – Collapse or walk towards left of the active item. 
- `RIGHT` Right – Uncollapse or walk towards right of the active item.
...
- range_select (bool) – Range Select, Select all between clicked and active items (optional)   Returns: 
