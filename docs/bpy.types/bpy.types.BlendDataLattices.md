...
Remove a lattice from the current blendfile
  ...
  Parameters:  
- lattice ([`Lattice`](bpy.types.Lattice.html#bpy.types.Lattice) | None) – Lattice to remove (never None) 
- do_unlink (bool) – Unlink all usages of this lattice before deleting it (WARNING: will also delete objects instancing that lattice data) (optional) 
- do_id_user (bool) – Decrement user counter of all data-blocks used by this lattice data (optional) 
- do_ui_user (bool) – Make sure interface does not reference this lattice data (optional)       tag(value) 
