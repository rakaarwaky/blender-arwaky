# bpy.types.FunctionNodeAlignRotationToVector
...
Axis to align to the vector (default `'X'`)

  
- `X` X – Align the X axis with the vector. 
- `Y` Y – Align the Y axis with the vector. 
- `Z` Z – Align the Z axis with the vector.   Type: 

Literal[‘X’, ‘Y’, ‘Z’]

      pivot_axis 

Axis to rotate around (default `'AUTO'`)
...
- `Z` Z – Rotate around the local Z axis.   Type: 
