# bpy.types.GeometryNodeProximity
...
Element of the target geometry to calculate the distance from (default `'FACES'`)

  
- `POINTS` Points – Calculate the proximity to the target’s points (faster than the other modes). 
- `EDGES` Edges – Calculate the proximity to the target’s edges. 
- `FACES` Faces – Calculate the proximity to the target’s faces.   Type: 
