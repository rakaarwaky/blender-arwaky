...
locale (str) – The ISO locale string to explode.
...
You should really rarely need to use this function in regular addon code, as all translation should be handled by Blender internal code. The only exceptions are strings containing formatting (like “File: %r”), but you should rather use `pgettext_iface()`/`pgettext_tip()` in those cases!
...
- msgctxt (str | None) – The translation context (defaults to BLT_I18NCONTEXT_DEFAULT).   Returns: 
...
- translations_dict (dict[str, dict[tuple[str, str], str]]) – A dictionary built like that: `{locale: {msg_key: msg_translation, ...}, ...}`       bpy.app.translations.unregister(module_name) 
