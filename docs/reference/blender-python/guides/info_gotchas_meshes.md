...
A common example is that exporters may access a mesh through `obj.data` (a [`bpy.types.Mesh`](bpy.types.Mesh.html#bpy.types.Mesh)) when the user is in Edit-Mode, where the mesh data is available but out of sync with the edit mesh.
...
- Explicitly update the mesh by calling [`bmesh.types.BMesh.to_mesh()`](bmesh.types.html#bmesh.types.BMesh.to_mesh). 
- Modify the script to support working on the edit-mode data directly, see: [`bmesh.from_edit_mesh()`](bmesh.html#bmesh.from_edit_mesh). 
...
- [`bpy.types.MeshPolygon`](bpy.types.MeshPolygon.html#bpy.types.MeshPolygon) – this is the data structure which now stores faces in Object-Mode (access as `mesh.polygons` rather than `mesh.faces`). 
- [`bpy.types.MeshLoopTriangle`](bpy.types.MeshLoopTriangle.html#bpy.types.MeshLoopTriangle) – the result of tessellating polygons into triangles (access as `mesh.loop_triangles`). 
- [`bmesh.types.BMFace`](bmesh.types.html#bmesh.types.BMFace) – the polygons as used in Edit-Mode.  
...
[`bpy.types.MeshPolygon`](bpy.types.MeshPolygon.html#bpy.types.MeshPolygon)

 

[`bpy.types.MeshLoopTriangle`](bpy.types.MeshLoopTriangle.html#bpy.types.MeshLoopTriangle)
...
Using the [`bmesh`](bmesh.html#module-bmesh) API is completely separate API from `bpy`, typically you would use one or the other based on the level of editing needed, not simply for a different way to access faces.
...
- Polygons are the most efficient way to create faces but the data structure is very rigid and inflexible, you must have all your vertices and faces ready and create them all at once. This is further complicated by the fact that each polygon does not store its own vertices, rather they reference an index and size in [`bpy.types.Mesh.loops`](bpy.types.Mesh.html#bpy.types.Mesh.loops) which are a fixed array too. 
- BMesh-faces are most likely the easiest way to create faces in new scripts, since faces can be added one by one and the API has features intended for mesh manipulation. While [`bmesh.types.BMesh`](bmesh.types.html#bmesh.types.BMesh) uses more memory it can be managed by only operating on one mesh at a time.    
