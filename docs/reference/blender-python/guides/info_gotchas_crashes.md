...
For safety, you should assume that undo and redo always invalidates all [`bpy.types.ID`](bpy.types.ID.html#bpy.types.ID) instances (Object, Scene, Mesh, Light, etc.), as well obviously as all of their sub-data.
...
One of the advantages with Blender’s library linking system is that undo can skip checking changes in library data since it is assumed to be static. Tools in Blender are not allowed to modify library data. But Python does not enforce this restriction.
...
Main reason is that those callbacks should be very fast, but additionally, it may for example create issues with undo/redo system (most operators store a history step, and editing an RNA property does so as well), trigger infinite update loops, and so on.
...
A dirty way of troubleshooting this is to set `sys.exit = None` and see what line of Python code is quitting, you could of course replace `sys.exit` with your own function but manipulating Python in this way is bad practice.
