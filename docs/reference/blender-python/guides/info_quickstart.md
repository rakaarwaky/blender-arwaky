...
- Enable [Developer Extra](https://docs.blender.org/manual/en/dev/editors/preferences/interface.html#bpy-types-preferencesview-show-developer-ui) and [Python Tooltips](https://docs.blender.org/manual/en/dev/editors/preferences/interface.html#bpy-types-preferencesview-show-tooltips-python). 
- The [Python Console](https://docs.blender.org/manual/en/dev/editors/python_console.html#bpy-types-spaceconsole) is great for testing one-liners; it has autocompletion so you can inspect the API quickly. 
...
Exact location depends on platform, see: [directory layout docs](https://docs.blender.org/manual/en/dev/advanced/blender_directory_layout.html#blender-directory-layout).   
...
This is an intentional part of the API design. The Blender Python API can’t create Blender data that exists outside the main Blender database (accessed through [`bpy.data`](bpy.data.html#module-bpy.data)), because this data is managed by Blender (save, load, undo, append, etc).

 

Data is added and removed via methods on the collections in [`bpy.data`](bpy.data.html#module-bpy.data), e.g:
...
Python can access properties on any data-block that has an ID (data that can be linked in and accessed from [`bpy.data`](bpy.data.html#module-bpy.data)). When assigning a property, you can pick your own names, these will be created when needed or overwritten if they already exist.
...
Using dictionaries allow to nest data into other data. There is a hard limit of 1024 levels to the supported nesting depth. Deeper nesting will lead to error messages and loss of data.
...
For a list of types that support custom properties see: [types supporting custom properties](bpy_types_custom_properties.html#bpy-types-custom-properties).
...
See [`bpy.context`](bpy.context.html#module-bpy.context) API reference.
...
Operators are tools generally accessed by the user from buttons, menu items or key shortcuts. From the user perspective they are a tool but Python can run these with its own settings through the [`bpy.ops`](bpy.ops.html#module-bpy.ops) module.
...
The [Operator Cheat Sheet](https://docs.blender.org/manual/en/dev/advanced/operators.html#bpy-ops-wm-operator-cheat-sheet) gives a list of all operators and their default values in Python syntax, along with the generated docs. This is a good way to get an overview of all Blender’s operators.
...
To avoid using try-except clauses wherever operators are called, you can call the operator’s own `poll()` function to check if it can run the operator in the current context.
...
- Move your cursor into the 3D Viewport, open the [Operator Search menu](https://docs.blender.org/manual/en/dev/interface/operators.html#bpy-ops-wm-search-menu), and type “Simple”. 
...
The class members with the `bl_` prefix are documented in the API reference [`bpy.types.Operator`](bpy.types.Operator.html#bpy.types.Operator).
...
[`bpy.types.bpy_struct`](bpy.types.bpy_struct.html#bpy.types.bpy_struct) is used for Blender data-blocks and collections. Also for data that contains its own attributes: collections, meshes, bones, scenes, etc.
...
Accessible from [`mathutils`](mathutils.html#module-mathutils) are vectors, quaternions, Euler angles, matrix and color types. Some attributes such as [`bpy.types.Object.location`](bpy.types.Object.html#bpy.types.Object.location), [`bpy.types.PoseBone.rotation_euler`](bpy.types.PoseBone.html#bpy.types.PoseBone.rotation_euler) and [`bpy.types.View3DCursor.location`](bpy.types.View3DCursor.html#bpy.types.View3DCursor.location) can be accessed as special math types which can be used together and manipulated in various useful ways.
