...
- [Application Timers (bpy.app.timers)](bpy.app.timers.html)     bpy.app.autoexec_fail 
...
In rare cases the default cache directory may not be available; in this case a temporary directory is used.
...
It is possible to run a script via the `--python` command line argument, before the blend file. This can register a load-post handler ([`bpy.app.handlers.load_post`](bpy.app.handlers.html#bpy.app.handlers.load_post)) that initializes the name-space. While this works for background tasks it has the downside that opening the file from the file selector won’t setup the name-space.
...
job_type (str) – job type in [Wm Job Type Items](bpy_types_enum_items/wm_job_type_items.html#rna-enum-wm-job-type-items).
