...
- `TextureNodeTree` Texture Node Editor – Edit textures using nodes.   Returns: 

New node tree data-block

  Return type: 

[`NodeTree`](bpy.types.NodeTree.html#bpy.types.NodeTree)
...
Remove a node tree from the current blendfile
  ...
  Parameters:  
- tree ([`NodeTree`](bpy.types.NodeTree.html#bpy.types.NodeTree) | None) – Node tree to remove (never None) 
...
- do_ui_user (bool) – Make sure interface does not reference this node tree (optional)       tag(value) 
