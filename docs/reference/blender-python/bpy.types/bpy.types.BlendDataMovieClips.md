...
Remove a movie clip from the current blendfile.
  ...
  Parameters:  
- clip ([`MovieClip`](bpy.types.MovieClip.html#bpy.types.MovieClip) | None) – Movie clip to remove (never None) 
- do_unlink (bool) – Unlink all usages of this movie clip before deleting it (optional) 
- do_id_user (bool) – Decrement user counter of all data-blocks used by this movie clip (optional) 
- do_ui_user (bool) – Make sure interface does not reference this movie clip (optional)       load(filepath, *, check_existing=False) 
