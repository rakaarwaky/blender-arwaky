...
- `INTEGRATE` Integrate – Integrate deformation from this modifier’s input with the mesh-cache coordinates (useful for shape keys).   Type: 
...
- `CUSTOM` Custom – Use the modifier’s own time evaluation.   Type: 
...
- `FACTOR` Factor – Control playback using a value between 0 and 1.   Type: 
