...
How to generate points from the input curve (default `'COUNT'`)

  
- `EVALUATED` Evaluated – Create points from the curve’s evaluated points, based on the resolution attribute for NURBS and Bézier splines. 
- `COUNT` Count – Sample each spline by evenly distributing the specified number of points. 
- `LENGTH` Length – Sample each spline by splitting it into segments with the specified length.   Type: 
