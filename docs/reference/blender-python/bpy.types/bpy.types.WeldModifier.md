...
Collapse edges without faces, cloth sewing edges (default False)
...
Limit below which to merge vertices (in [0, inf], default 0.001)
...
Mode defines the merge rule (default `'ALL'`)

  
- `ALL` All – Full merge by distance. 
- `CONNECTED` Connected – Only merge along the edges.   Type: 
