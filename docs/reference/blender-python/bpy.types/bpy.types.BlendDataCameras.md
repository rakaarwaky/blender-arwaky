...
Remove a camera from the current blendfile
  ...
  Parameters:  
- camera ([`Camera`](bpy.types.Camera.html#bpy.types.Camera) | None) – Camera to remove (never None) 
...
- do_ui_user (bool) – Make sure interface does not reference this camera (optional)       tag(value) 
