...
- `4D` 4D – Use the 4D vector (X, Y, Z, W) as input.   Type: 
...
- `MULTIFRACTAL` Multifractal – More uneven result (varies with location), more similar to a real terrain. 
- `RIDGED_MULTIFRACTAL` Ridged Multifractal – Create sharp peaks. 
- `HYBRID_MULTIFRACTAL` Hybrid Multifractal – Create peaks and valleys with different roughness values. 
- `FBM` fBM – The standard fractal Perlin noise. 
- `HETERO_TERRAIN` Hetero Terrain – Similar to Hybrid Multifractal creates a heterogeneous terrain, but with the likeness of river channels.   Type: 
