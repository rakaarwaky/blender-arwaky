# bpy.types.GeometryNodeSetMeshNormal
...
Literal[[Attribute Domain Only Mesh No Edge Items](bpy_types_enum_items/attribute_domain_only_mesh_no_edge_items.html#rna-enum-attribute-domain-only-mesh-no-edge-items)]
...
- `TANGENT_SPACE` Tangent Space – Store normals in a deformation dependent custom transformation space. This method is slower, but can be better when subsequent operations change the mesh without handling normals specifically..   Type: 
