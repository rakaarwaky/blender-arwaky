...
Remove a text from the current blendfile
  ...
  Parameters:  
- text ([`Text`](bpy.types.Text.html#bpy.types.Text) | None) – Text to remove (never None) 
...
- do_ui_user (bool) – Make sure interface does not reference this text (optional)       load(filepath, *, internal=False) 
...
- internal (bool) – Make internal, Make text file internal after loading (optional)   Returns: 
