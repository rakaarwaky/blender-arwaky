...
Remove a collection from the current blendfile
  ...
  Parameters:  
- collection ([`Collection`](bpy.types.Collection.html#bpy.types.Collection) | None) – Collection to remove (never None) 
...
- do_ui_user (bool) – Make sure interface does not reference this collection (optional)       tag(value) 
