...
Remove a curves data-block from the current blendfile
  ...
  Parameters:  
- curves ([`Curves`](bpy.types.Curves.html#bpy.types.Curves) | None) – Curves data-block to remove (never None) 
- do_unlink (bool) – Unlink all usages of this curves before deleting it (WARNING: will also delete objects instancing that curves data) (optional) 
- do_id_user (bool) – Decrement user counter of all data-blocks used by this curves data (optional) 
- do_ui_user (bool) – Make sure interface does not reference this curves data (optional)       tag(value) 
