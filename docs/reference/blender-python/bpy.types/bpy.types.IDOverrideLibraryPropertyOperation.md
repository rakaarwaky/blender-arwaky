...
- `IDPOINTER_ITEM_USE_ID` ID Item Use ID Pointer – RNA collections of IDs only, the reference to the item also uses the ID pointer itself, not only its name.   Type: 
...
- `CUSTOM` Custom – Custom operation, specific to a RNA property, and handled through dedicated callbacks (used in specific cases, e.g. to handle data not actually exposed in RNA).   Type: 
...
- [`IDOverrideLibraryPropertyOperations.remove`](bpy.types.IDOverrideLibraryPropertyOperations.html#bpy.types.IDOverrideLibraryPropertyOperations.remove)
