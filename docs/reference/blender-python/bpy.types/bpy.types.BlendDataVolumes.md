...
Remove a volume from the current blendfile
  ...
  Parameters:  
- volume ([`Volume`](bpy.types.Volume.html#bpy.types.Volume) | None) – Volume to remove (never None) 
- do_unlink (bool) – Unlink all usages of this volume before deleting it (WARNING: will also delete objects instancing that volume data) (optional) 
- do_id_user (bool) – Decrement user counter of all data-blocks used by this volume data (optional) 
- do_ui_user (bool) – Make sure interface does not reference this volume data (optional)       tag(value) 
