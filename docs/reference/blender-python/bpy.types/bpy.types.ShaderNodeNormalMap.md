...
- `DISPLACED` Displaced Base – Apply normal map relative to the displaced surface, for normal maps that are to be applied in addition to the displacement map.   Type: 
...
- `DIRECTX` DirectX – Normal map uses DirectX convention, with Y axis in the green channel pointing down.   Type: 
...
- `BLENDER_WORLD` Blender World Space – World space normal mapping, compatible with Blender render baking.   Type: 
