...
Additionally, if the operator provides operator properties that need to be accessible to the user, the `ImportHelper.invoke_popup` method can be used to show a dialog leveraging the standard [`Operator.draw`](bpy.types.Operator.html#bpy.types.Operator.draw) method for layout and display.
...
- [VIEW3D_FH_vdb_volume(FileHandler)](bpy.types.VIEW3D_FH_vdb_volume.html)     class bpy.types.FileHandler(bpy_struct) 
