...
Remove an action from the current blendfile
  ...
  Parameters:  
- action ([`Action`](bpy.types.Action.html#bpy.types.Action) | None) – Action to remove (never None) 
...
- do_ui_user (bool) – Make sure interface does not reference this action (optional)       tag(value) 
