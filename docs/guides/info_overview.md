...
Blender’s embedded interpreter provides a typical Python environment, so code from tutorials on how to write Python scripts can also be run with Blender’s interpreter. Blender provides its Python modules, such as `bpy` and [`mathutils`](mathutils.html#module-mathutils), to the embedded interpreter so they can be imported into a script and give access to Blender’s data, classes, and functions. Scripts that deal with Blender data will need to import the modules to work.
...
On startup Blender scans the `scripts/startup/` directory for Python modules and imports them. The exact location of this directory depends on your installation. See the [directory layout docs](https://docs.blender.org/manual/en/dev/advanced/blender_directory_layout.html#blender-directory-layout).
...
Some of Blender’s functionality is best kept optional, alongside scripts loaded at startup there are add-ons which are only loaded on startup if enabled from the user preferences. Add-ons are typically distributed as extensions.

 

The only difference between add-on extensions and built-in Python modules is that add-ons must include a `blender_manifest.toml` which Blender uses to read metadata such as name, author, tags and project link. The User Preferences add-on listing uses this to display information about each add-on. See [Creating Extensions](https://docs.blender.org/manual/en/dev/advanced/extensions/index.html#extensions-index) for details.
...
- [`bpy.types.Panel`](bpy.types.Panel.html#bpy.types.Panel) 
- [`bpy.types.Menu`](bpy.types.Menu.html#bpy.types.Menu) 
- [`bpy.types.Operator`](bpy.types.Operator.html#bpy.types.Operator) 
- [`bpy.types.PropertyGroup`](bpy.types.PropertyGroup.html#bpy.types.PropertyGroup) 
- [`bpy.types.KeyingSet`](bpy.types.KeyingSet.html#bpy.types.KeyingSet) 
...
First note that it defines a subclass as a member of [`bpy.types`](bpy.types.html#module-bpy.types), this is common for all classes which can be integrated with Blender and is used to distinguish an Operator from a Panel when registering.
...
So once the class is registered with Blender, instancing the class and calling the functions is left up to Blender. In fact you cannot instantiate these classes from the script as you would expect with most Python APIs. To run operators you can call them through the operator API, e.g:
...
The lifetime of class instances is usually very short (also see the [dedicated section](info_gotchas_internal_data_and_python_objects.html#blender-py-objects-life-time)), a panel for example will have a new instance for every redraw. Some other types, like [`bpy.types.Operator`](bpy.types.Operator.html#bpy.types.Operator), have an even more complex internal handling, which can lead to several instantiations for a single operator execution.

 

There are a few cases where defining `__init__()` does make sense, e.g., when sub-classing a [`bpy.types.RenderEngine`](bpy.types.RenderEngine.html#bpy.types.RenderEngine). When doing so, the parent matching function must always be called, otherwise Blender’s internal initialization won’t happen properly:
...
Unfortunately, the error message, generated in case the expected constructor is not called, can be fairly cryptic and unhelpful. Generally they should be about failure to create a (python) object:
...
Registering a class with Blender results in the class definition being loaded into Blender, where it becomes available alongside existing functionality. Once this class is loaded you can access it from [`bpy.types`](bpy.types.html#module-bpy.types), using the `bl_idname` rather than the class’s original name.
...
There are some exceptions to this for class names which aren’t guaranteed to be unique. In this case use: [`bpy.types.Struct.bl_rna_get_subclass_py()`](bpy.types.Struct.html#bpy.types.Struct.bl_rna_get_subclass_py).
...
In some cases the specifier for data may not be in Blender, for example an external render engine’s shader definitions, and it may be useful to define them as types and remove them on the fly.
