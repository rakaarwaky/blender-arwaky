...
Python threading with Blender only works properly when the threads finish up before the script does, for example by using `threading.Thread.join()`. In other words, they can only be used while the main Blender thread is blocked from running.
...
This is an example of an unsupported case, using a timer that runs many times a second:
