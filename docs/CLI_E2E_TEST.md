...
Action yang tervalidasi secara langsung adalah `get_scene_info`, `get_object_info`, dan `render`. Action `execute_blender_code` adalah canonical dispatcher action dan dapat diakses melalui subcommand CLI `run-code` serta MCP `execute_command`.
