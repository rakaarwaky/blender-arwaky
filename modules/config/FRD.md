...
- **Rules**: Traverse nested structure by segments. Missing key/intermediate → default. Empty path → full snapshot. Returned values immutable or deep-copied. Numeric segments access list positions (out-of-range → default). `\.` always escapes a literal dot inside a path segment. Thread-safe, lock-free after init. Never triggers file/env reads per request. Type mismatch → default (permissive) or TypeError (strict). Defaults never mutated by retrieval.
