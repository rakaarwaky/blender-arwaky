...
class Registry:
    ...
    def assert_no_active(self) -> CliResultVo | None:
        ...
        if self._state.active_entity:
            return CliResultVo(
                success=False,
                error=f"Blender is currently active using '{self._state.active_entity}'. Close it first.",
                category="state",
                ref="cli-409",
            )
        return None
    ...
    def assert_active(self, filepath: str) -> CliResultVo | None:
        ...
        if self._state.active_entity != filepath:
            return CliResultVo(
                success=False,
                error=f"Entity '{filepath}' is not registered. Active entity: '{self._state.active_entity}'.",
                category="state",
                ref="cli-409",
            )
        return None
