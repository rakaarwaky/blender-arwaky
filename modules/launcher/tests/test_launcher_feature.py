...
def test_fr_lau_001_registers_override_executable():
    ...
    # python is executable; locator accepts it as a candidate (validation only
    # checks file+X_OK; name-authenticity is deferred to the runner).
    assert res.source == RegistrationSource.OVERRIDE


def test_fr_lau_001_no_candidate_returns_error():
    feat = create_launcher_feature(LauncherConfigVO())
    res = feat.locate_and_register(LauncherConfigVO())  # no override, no config path
...
class _FakeStatus:
    """Controllable status backend for launch/shutdown tests."""
