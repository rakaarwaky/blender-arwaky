...
class TestHealthCompositionStatusDerivation:
    ...
    def test_all_healthy_returns_overall_healthy(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "healthy"

    def test_any_unhealthy_returns_overall_unhealthy(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="unhealthy", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "unhealthy"

    def test_any_failed_returns_overall_unhealthy(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="failed", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "unhealthy"

    def test_any_unreachable_returns_overall_unhealthy(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="unreachable", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "unhealthy"

    def test_any_degraded_returns_overall_degraded(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="degraded", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "degraded"

    def test_config_invalid_returns_unhealthy(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="healthy", config_valid=False, job_capacity_available=True)))
        assert result.overall_status == "unhealthy"

    def test_job_capacity_exhausted_returns_degraded(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="healthy", config_valid=True, job_capacity_available=False)))
        assert result.overall_status == "degraded"

    def test_unknown_statuses_are_degraded(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="unknown", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "degraded"
...
class TestStalenessIndicators:
    ...
    def test_unknown_gateway_populates_staleness_delta(self) -> None:
        cap = _make_composer()
        result = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="unknown", config_valid=True, job_capacity_available=True)))
        assert result.overall_status == "degraded"
...
class TestCompositionIdempotency:
    ...
    def test_identical_input_within_freshness_returns_cached(self) -> None:
        cap = _make_composer()
        r1 = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
        r2 = asyncio.run(cap.compose_health(request=_request(launcher_status="healthy", gateway_status="healthy", config_valid=True, job_capacity_available=True)))
