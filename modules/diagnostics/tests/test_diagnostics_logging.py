...
class TestLogRedaction:
    ...
    def test_redacts_password_in_message(self) -> None:
        cap = _make_policy()
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="password=secret123"))
    ...
    def test_redacts_api_key_in_fields(self) -> None:
        cap = _make_policy()
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="test", fields={"api_key": "abc123def456"}))
        assert "REDACTED" in str(cap._buffer[-1]["fields"])

    def test_redacts_token_in_message(self) -> None:
        cap = _make_policy()
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="token=ghp_abc123def456ghi789jkl012mno345"))
        assert "REDACTED" in cap._buffer[-1]["message"]

    def test_debug_does_not_bypass_redaction(self) -> None:
        cap = _make_policy()
        asyncio.run(cap.log_record(level="debug", source_feature="cli", message="password=debug_secret"))
        assert "REDACTED" in cap._buffer[-1]["message"]
...
class TestBackpressure:
    ...
    def test_bounded_buffer_drops_oldest(self) -> None:
        ...
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="first"))
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="second"))
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="third"))
        asyncio.run(cap.log_record(level="info", source_feature="cli", message="fourth"))
    ...
    def test_drop_counter_increments(self) -> None:
        ...
        for _ in range(5):
            asyncio.run(cap.log_record(level="info", source_feature="cli", message=f"log {_}"))
        assert cap.get_drop_counter() >= 3

    def test_redacted_count_in_result(self) -> None:
        cap = _make_policy()
        result = asyncio.run(cap.log_record(level="info", source_feature="cli", message="password=secret"))
