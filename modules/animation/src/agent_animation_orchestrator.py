...
class AnimationOrchestrator(IWaveFeatureAggregate):
    ...
    def __init__(
        ...
        retarget_executor: IWaveFeatureProtocol | None = None,
    ...
    async def create_nla_track(self, armature_name: str, track_name: str, is_solo: bool = False, is_muted: bool = False):
        return await self._nla_executor.create_nla_track(armature_name, track_name, is_solo, is_muted)

    async def add_nla_strip(self, armature_name: str, track_name: str, action_name: str, strip_name: str, frame_start: float, scale: float = 1.0, repeat: float = 1.0, blend_in: float = 0.0, blend_out: float = 0.0, influence: float = 1.0, blend_type: str = "REPLACE", extrapolation: str = "HOLD", reversed: bool = False):
        return await self._nla_executor.add_nla_strip(armature_name, track_name, action_name, strip_name, frame_start, scale, repeat, blend_in, blend_out, influence, blend_type, extrapolation, reversed)

    async def set_nla_strip(self, armature_name: str, track_name: str, strip_name: str, frame_start: float | None = None, scale: float | None = None, repeat: float | None = None, blend_in: float | None = None, blend_out: float | None = None, influence: float | None = None, blend_type: str | None = None, extrapolation: str | None = None, reversed: bool | None = None):
        return await self._nla_executor.set_nla_strip(armature_name, track_name, strip_name, frame_start, scale, repeat, blend_in, blend_out, influence, blend_type, extrapolation, reversed)

    async def set_animation_layer(self, armature_name: str, track_name: str, blend_type: str | None = None, influence: float | None = None, is_solo: bool | None = None, is_muted: bool | None = None):
        return await self._nla_executor.set_animation_layer(armature_name, track_name, blend_type, influence, is_solo, is_muted)
    ...
    async def bake_nla_assembly(self, armature_name: str, frame_start: int, frame_end: int, step: int = 1, output_action: str = "Wave5_Baked_Action", clear_constraints: bool = False, clear_nla: bool = False):
        return await self._nla_executor.bake_nla_assembly(armature_name, frame_start, frame_end, step, output_action, clear_constraints, clear_nla)
