...
class TestBlockedModuleImports:
    ...
    def test_os_import_blocked(self) -> None:
        ...
        assert any("blocked_module_import" in v.category for v in res.violations)  # nosec B101
    ...
    def test_multiple_blocked_imports(self) -> None:
        ...
        res = _validate(cap, "import os\nimport subprocess\nimport sys")
        assert res.allowed is False  # nosec B101
        assert len(res.violations) >= 2  # nosec B101
...
class TestBlockedFunctionCalls:
    ...
    def test_eval_call_blocked(self) -> None:
        ...
        assert res.allowed is False  # nosec B101
        assert any("blocked_function_call" in v.category for v in res.violations)  # nosec B101
...
class TestSizeLimits:
    ...
    def test_code_exceeding_size_limit_rejected(self) -> None:
        ...
        assert res.allowed is False  # nosec B101
        assert any("size_limit" in v.category for v in res.violations)  # nosec B101
...
class TestStrictMode:
    ...
    def test_syntax_error_strict_rejected(self) -> None:
        ...
        assert res.allowed is False  # nosec B101
        assert any("syntax_error" in v.category for v in res.violations)  # nosec B101
...
class TestDisabledValidation:
    ...
    def test_disabled_validation_has_audit_metadata(self) -> None:
        ...
        assert res.audit_metadata.get("rule") == "validation_disabled_override"  # nosec B101

    def test_disabled_validation_has_warning(self) -> None:
        """FR-SEC-003: disabled validation includes redacted metadata warning."""
        cap = _make_validator(SecurityPolicyVO(code_validation_enabled=False))
        res = _validate(cap, "import os")
        assert isinstance(res.redacted_metadata, dict)  # nosec B101
...
class TestEdgeCases:
    ...
    def test_empty_code_payload(self) -> None:
        ...
        res = _validate(cap, "")
        assert res.allowed is False  # nosec B101
        assert any("empty_code" in v.category for v in res.violations)  # nosec B101
    ...
    def test_violation_has_category(self) -> None:
        ...
        cap = _make_validator()
        res = _validate(cap, "import os")
        assert len(res.violations) >= 1  # nosec B101
        assert res.violations[0].category == "blocked_module_import"  # nosec B101
    ...
    def test_violation_has_description(self) -> None:
        ...
        res = _validate(cap, "import os")
        assert len(res.violations) >= 1  # nosec B101
        assert "Blocked import" in res.violations[0].description  # nosec B101
...
class TestAuditMetadata:
    ...
    def test_size_limit_has_audit_metadata(self) -> None:
        ...
        assert isinstance(res.audit_metadata, dict)  # nosec B101
        assert res.audit_metadata.get("rule") == "code_oversized"  # nosec B101
...
class TestRepresentation:
    ...
    def test_code_validator_repr(self) -> None:
        ...
        assert "CodeValidator" in repr(cap)  # nosec B101
...
def test_custom_blocked_construct_rejects_import() -> None:
    ...
    assert any(v.category == "blocked_module_import" for v in result.violations)  # nosec B101
