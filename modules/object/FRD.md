...
- **Rules**: Object must already exist in scene. Not-yet-imported → rejected with guidance to asset feature. For the CLI `place-asset` surface, `asset_id` is the exact Blender object name returned by the prior import/asset-resolution step; the operation never silently downloads or resolves a provider ID. Reference resolution is deterministic: exact name → qualified path/collection. Ambiguous → ambiguous reference error. Transform values must be finite 3-vectors. CLI rotation values are degrees and are converted to Blender radians at the addon boundary. Zero scale rejected unless explicitly allowed. Preserves identity (no rename/duplicate). Idempotent for identical ref + transform. Locked channels respected unless explicit override.
