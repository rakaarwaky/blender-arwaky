"""Shared test fixtures for the object feature test suite.

Mirror the security suite shim: ``modules.shared.src.__init__`` transitively
imports the shared ``mcp`` subpackage (``modules.shared.src.mcp.*``), which has
no ``__init__.py`` and cannot be imported without injecting a lightweight ``mcp``
