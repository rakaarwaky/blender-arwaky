...
class SceneCleanupExecutor(ISceneCleanupProtocol):
    ...
    async def cleanup_scene(self, request: SceneCleanupVO) -> SceneCleanupVO:
        ...
        try:
            ...
            if request.dry_run:
                ...
                logger.info("scene_cleanup_dry_run_completed event=%s", event)
                ...
                return SceneCleanupVO(
                    ...
                    preserved_count=metrics.preserved_count,
                    skipped_count=metrics.skipped_count,
                    removed_object_references=metrics.removed_object_references,
                    preserved_object_references=metrics.preserved_object_references,
                    skipped_object_references=metrics.skipped_object_references,
                    message=Prompt(
            ...
            logger.info("scene_cleanup_completed event=%s", event)
            ...
            return SceneCleanupVO(
                ...
                preserved_object_references=metrics.preserved_object_references,
                skipped_object_references=metrics.skipped_object_references,
                message=Prompt(f"Cleanup completed: {metrics.removed_count} removed"),
            )

        except Exception as exc:
            logger.exception("Scene cleanup failed")

            failed_event = SceneCleanupFailedEvent(
                correlation_id=request.correlation_id,
            ...
            logger.info("scene_cleanup_failed event=%s", failed_event)
    ...
    def _resolve_policy(self, request: SceneCleanupVO) -> SceneCleanupPolicyVO:
        ...
        return SceneCleanupPolicyVO(
            mode=request.mode,
            preserve_cameras=PRESERVATION_CAMERA in preservation,
            preserve_lights=(PRESERVATION_LIGHT in preservation or PROTECTED_OBJECT_POLICY_LIGHTS),
            include_hidden_objects=request.include_hidden_objects,
            child_handling_policy=request.child_handling_policy,
            dependent_handling_policy=request.dependent_handling_policy,
